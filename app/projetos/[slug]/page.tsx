      ...
      <section className="pt-20">
        <div className="relative min-h-[78vh] flex items-end">
          ...
          <div className="container-x relative z-10 pb-14">
            ...
            <h1 className="display text-5xl md:text-7xl mt-3">{p.title}</h1>
            <div className="mt-4 text-white/55">{p.location}{p.area ? ` • ${p.area}` : ""}</div>
          </div>
        </div>
      </section>
  ...
  />
          ...
          <aside className="border-l border-white/10 pl-8">
            ...
            <div className="mt-6 space-y-4">
              {p.highlights.map(x => <div key={x} className="rule pt-4">{x}</div>)}
            </div>

            <div className="eyebrow mt-12">Desafios técnicos</div>
            <p className="muted mt-4">Adicionar informações específicas aprovadas pela Corddiali.</p>
      ...
      <section className="border-t border-white/10 py-16">
        <div className="container-x flex flex-col md:flex-row gap-5 justify-between md:items-center">
          <div className="eyebrow">Próximo projeto</div>
          <Link href={`/projetos/${next.slug}`} className="text-2xl font-serif">{next.title} →</Link>
        </div>
      </section>
      <Footer />
    </main>
